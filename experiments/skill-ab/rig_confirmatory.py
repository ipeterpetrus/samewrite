...
def one(task, arm, rep, model, timeout, ledger):
    ...
    prompt=PROMPT if arm!="oneline" else ONELINER["caveman"]+" "+PROMPT
    ...
    return dict(run_id=run_id, task=task, arm=arm, rep=rep, rc=rc, sec=round(time.time()-t0,1),
...
def main():
    ...
    for rep in range(a.repeat):
        for task in sorted(T):
            for arm in ("plain","oneline","mode"):
                r=one(task,arm,rep,a.model,a.timeout,a.ledger)
